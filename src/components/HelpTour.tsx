...
const SLIDES: Slide[] = [
  ...
  },
  {
    ...
    verbs: [
      ...
      { name: 'project task', what: 'creating a task scoped to a project now uses the SAME composer as everything else — there is no separate flow. On Home, drill into a project (l/Enter) and use the ＋ New task button in its folder header (or type “new task”) to open the composer with that project PRE-SELECTED; the project rides the create and carries its folder + cascading instructions as agent context. Opening + New task from a folder that belongs to a project also auto-attaches that project. Either way you can override the project or pick None.' },
