...
const SLIDES: Slide[] = [
  ...
  },
  {
    ...
    verbs: [
      ...
      { name: 'pipeline roster', what: 'a top-level task with children whose OWN body carries a chain (see task templates) renders as a SUBTABLE right under its summary row, with grouped headers per task-def (each step’s IN / OUT / required columns) — a plain task renders as a normal row, so a mixed project shows both side by side · INPUT cells are editable INLINE (the edit rewrites just that child’s values block, preserving its notes) and OUTPUT cells are read-only · a step whose needed-when is unmet hatches its whole column group as n/a · CLICK any cell to open that step’s child task; click the title to open the meta-parent rollup · bodies are fetched lazily and cached PHI-safely (an unfetched cell shows a loading em-dash)' },
