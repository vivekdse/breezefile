...
const SLIDES: Slide[] = [
  ...
  },
  {
    ...
    verbs: [
      ...
      { name: 'new-home', chord: ':new-home / :nh', what: 'agent work monitor for a project — an amber Approval Bar surfaces every task blocked on you first, Hero Stats (Done / In Progress / Needs You / Failed) double as filter toggles, the Roster table lists every task with per-project custom columns, who acted last (🤖 / 👤 / 🤖+👤 — derived from the audit trail when available), and one-click Answer/Retry, with a Search box + status pills to filter it — focus a row and use j/k or ↑/↓ to move between rows and Enter to open the detail dialog (scoped to the table, so it never steals j/k from the rest of the app) — the box takes plain words OR a SQL-like query over task fields (e.g. `status in (needs, failed) and repeatable`, `due < now+7d`), the same no-eval DSL the tag filters use; the copilot drives it via set_roster_filter (status/text) and query_roster (structured), and an Outcomes rollup below groups finished work (Done vs Failed) with a one-line result per task · click any row to open the detail dialog: full evidence log, structured outcome, cancel/retry, and a talk-back box to message the agent · Customize opens an INLINE per-project template panel (right in the page, not a modal): custom fields, roster columns, Repeatable Tasks (reusable task templates you Run now or put on a Daily/Weekly/Monthly schedule), Chains (an ordered sequence of tasks — add a repeatable from the dropdown or a free-form step, reorder, then Run chain to spawn linked tasks), approval rules, and steps · every one of these is also drivable by the copilot, which shows the same panel as it edits · + New Task opens the canonical Task form (the same one the task verb / Sidebar / copilot create_task open) and pops the copilot chat alongside it, so you can fill the form by hand or drive it conversationally' },
      { name: 'task templates', what: 'a project’s Customize panel has a TASKS tab (New Home → Customize → Tasks) where you build a TEMPLATE: an ordered CHAIN of task-defs (steps). Each task-def owns its own INPUT fields (a human answers these when a job is created) and OUTPUT fields (the agent produces these) — the template holds no fields of its own, forms and columns are aggregated from every task-def in order · mark an output REQUIRED to make it that step’s EVIDENCE (the step isn’t complete until the agent submits it) · a step can carry a needed-when CONDITION (op ==/!=/</> against an UPSTREAM output) so it only runs when the condition holds — an unmet condition marks the step Not needed and its cells render hatched n/a · definitions (keys/labels/types/conditions) are NON-PHI config; the VALUES entered later ride only on the tasks (encrypted), never in the template' },
      { name: 'create a job (from a template)', what: 'a templated project’s + New Task uses the SAME canonical composer — it adds a Template step, then asks every task-def’s INPUT questions grouped by owning step, and shows the outputs read-only (“the agent will produce: …, required”) · saving spawns a JOB: one meta-parent task carrying the ordered task-def list, plus one linearly-chained CHILD task per step (parent + depends-on wired so the steps run in order), each child seeded with its own input values and its required-output/evidence instructions for the agent · a blank (non-template) task is byte-identical to before' },
      { name: 'pipeline roster', what: 'a templated project renders its roster as a PIPELINE table: one row per job, with grouped headers per task-def (each step’s IN / OUT / required columns sit between Status and Last Action) · INPUT cells are editable INLINE (the edit rewrites just that child’s values block, preserving its notes) and OUTPUT cells are read-only · a step whose needed-when is unmet hatches its whole column group as n/a · CLICK any cell to open that step’s child task; click the title to open the meta-parent rollup · child bodies are fetched lazily and cached PHI-safely (an unfetched cell shows a loading em-dash)' },
      { name: 'outputs, evidence & pipeline rollup', what: 'the task detail dialog surfaces the template loop: a CHILD task shows an OUTPUTS section (its declared output fields vs what the agent submitted, with an N-of-M required line) and its required outputs appear as first-class EVIDENCE entries in the log — an unmet requirement reads “awaiting agent — owes N required output(s)” · a META-PARENT (job) task shows a PIPELINE rollup: the ordered steps, each with a status pill (Done / In progress / Pending / Not needed) and a one-line outcome, every row clicking through to its child task' },
      ...
      { name: 'author custom form behavior (chat)', what: 'in New Home, ask the copilot to build custom FORM BEHAVIOR in plain English (e.g. "show a diagnosis typeahead only when the visit type is follow-up, and flag an age over 120") — grounded with the effect contract and the approved SavedQueries it can bind typeahead fields to, it DRAFTS extra fields[] + a PURE logic function (draft_form_extension), then you TEST it against sample values and see the returned effects (test_form_logic) and APPROVE it on a mandatory review card that shows the exact fields + logic (approve_form_extension) · once approved the + New Task form RENDERS those fields with the app’s own trusted widgets and, on any change, runs the logic server-side and applies its four allowlisted effects — set a value, show/hide a field, replace select options, or show an inline validation error · the client never eval’s the logic and never injects markup · iterate with new_form_extension_version (clone → new draft) · a form with no approved extension is unchanged · fields/logic are config (not PHI)' },
      ...
      { name: 'copilot', chord: '⌘/ or Ctrl+/', what: 'a persistent AI chat sidebar available on every surface — ask it to navigate the app (e.g. open New Home), create tasks or projects, filter the roster, or open a specific task · it drives the same agent-work affordances you click on Home: answer an agent\'s pending question (answer_task, or bulk_answer_tasks to send one reply to many at once — the Approval Bar\'s Answer/Reject/"Approve selected"), retry or start a task (retry_task — the "Retry" button), and message the agent working a task (message_agent — the detail dialog\'s "Send message"); each of these shows an approve card before it sends · it can also open the Customize panel (open_customize) and fully edit the template: add/edit/reorder/delete fields, columns, steps, approval rules, Repeatable Tasks (and Run now / schedule them), and Chains (add tasks, reorder, Run chain) — it reveals the same panel you see as it works, so it is provably operating the surface, not a hidden copy · in New Home\'s + New Task modal, the same agent conducts the interview one question at a time and fills the form live · needs an Anthropic API key configured (ANTHROPIC_API_KEY or the same key store the tag-assist NL box uses); shows a quiet setup hint instead when none is set · chat text is not persisted to disk' },
    ],
