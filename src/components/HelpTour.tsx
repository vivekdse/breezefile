...
const SLIDES: Slide[] = [
  ...
  },
  {
    ...
    verbs: [
      ...
      { name: 'copilot', chord: '⌘/ or Ctrl+/', what: 'a persistent AI chat sidebar available on every surface — ask it to navigate the app (e.g. open New Home), create tasks or projects (including a chained task), filter the roster, or open a specific task · it drives the same agent-work affordances you click on Home: answer an agent\'s pending question (answer_task, or bulk_answer_tasks to send one reply to many at once), retry or start a task (retry_task — the "Retry" button), and message the agent working a task (message_agent — the detail dialog\'s "Send message"); each of these shows an approve card before it sends · needs an Anthropic API key configured (ANTHROPIC_API_KEY or the same key store the tag-assist NL box uses); shows a quiet setup hint instead when none is set · chat text is not persisted to disk' },
    ],
