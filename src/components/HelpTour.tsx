...
const SLIDES: Slide[] = [
  ...
  },
  {
    ...
    verbs: [
      ...
      { name: 'new-home', chord: ':new-home / :nh', what: 'agent work monitor for a project — Hero Stats (Done / In Progress / Needs You / Failed) double as filter toggles, the Roster table lists every task with who acted last (🤖 / 👤 / 🤖+👤 — derived from the audit trail when available), and one-click Answer / Retry / ▶ Start (Start claims + launches a Claude session for a queued, unclaimed task — the same play-button mechanism as the Tasks page), with a Search box + status pills to filter it — focus a row and use j/k or ↑/↓ to move between rows and Enter to open the detail dialog (scoped to the table, so it never steals j/k from the rest of the app) — the box takes plain words OR a SQL-like query over task fields (e.g. `status in (needs, failed)`, `due < now+7d`), the same no-eval DSL the tag filters use; the copilot drives it via set_roster_filter (status/text) and query_roster (structured); whenever a status bucket and/or a search/query is active a Filtering chip bar sits above the roster showing each active filter in human-readable form (a ⚡ marks a structured query, ⚠ an invalid one) — click a chip’s ✕ to clear just that filter or Clear all to reset the roster, so what the copilot filters is always visible and clearable by hand — and an Outcomes rollup below groups finished work (Done vs Failed) with a one-line result per task · click any row to open the detail dialog: full evidence log, structured outcome, cancel / retry / ▶ start, and a talk-back box to message the agent · a project carries no separate configuration — it is just a category + this derived view · + New Task opens the canonical Task form (the same one the task verb / Sidebar / copilot create_task open) and pops the copilot chat alongside it, so you can fill the form by hand or drive it conversationally' },
      { name: 'task templates', what: 'the canonical Task form has a "New Chained Task" entry point where you define a CHAIN right there: an ordered sequence of task-defs (steps), inline, OR copied from an existing chained task and edited before you submit — there is no separate project template to save it to. Each task-def owns its own INPUT fields (a human answers these when a job is created) and OUTPUT fields (the agent produces these) · mark an output REQUIRED to make it that step’s EVIDENCE (the step isn’t complete until the agent submits it) · a step can carry a needed-when CONDITION (op ==/!=/</> against an UPSTREAM output) so it only runs when the condition holds — an unmet condition marks the step Not needed and its cells render hatched n/a · the whole chain rides on the created job’s own task body (self-describing), never a project pref — definitions (keys/labels/types/conditions) are NON-PHI config; the VALUES entered later ride only on the tasks (encrypted), never in a template · the composer DEFINES input fields but never asks for their VALUES at create time — a plain task that defines inputs offers a “Press F to fill inputs now” escape hatch on the success flash (a values-only walk that writes them via the same Inputs data-bag path the drawer uses); otherwise fill them later in the drawer’s Inputs editor / roster cells or the from-template flow' },
      ...
      { name: 'create a job (from a chained task)', what: 'submitting a "New Chained Task" spawns a JOB: one meta-parent task carrying the ordered task-def list in full (self-describing — any surface can reconstruct the chain from this one task), plus one linearly-chained CHILD task per step (parent + depends-on wired so the steps run in order), each child carrying its required-output/evidence instructions for the agent · creation DEFINES each step’s input fields but does NOT ask for their VALUES — the job instantiates with empty inputs and you fill them later inline in the roster / the drawer’s Inputs editor (or via the from-template flow) · a blank (non-chained) task is byte-identical to before' },
