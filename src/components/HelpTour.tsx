...
const SLIDES: Slide[] = [
  ...
  },
  {
    ...
    verbs: [
      ...
      { name: 'Start a TypeBuild task', what: '▸ Start on a pending TypeBuild row launches an interactive Claude session in a new tab, pre-wired to the task — you never type a command · the agent claims the task in-session · enabled only when you’re signed in and the onboarding prerequisites pass (Settings → TypeBuild: Claude Code + Chrome); the disabled tooltip says what’s missing · a first-run hint reminds you to approve the TypeBuild sign-in (/mcp) when Claude asks · when the session ends while you still hold the claim, a prompt offers Release · PHI-safe — the tab is labelled generically (“TypeBuild task …”) and terminal scrollback is never written to disk' },
