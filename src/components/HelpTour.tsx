...
const SLIDES: Slide[] = [
  ...
  },
  {
    ...
    verbs: [
      ...
      { name: 'Start a TypeBuild task', what: '▸ Start on a pending TypeBuild row launches an interactive Claude session in a new tab, pre-wired to the task — you never type a command · the session starts already authenticated (no /mcp sign-in prompt) and the agent claims the task in-session · enabled only when you’re signed in and the onboarding prerequisites pass (Settings → TypeBuild: Claude Code + Chrome); the disabled tooltip says what’s missing · if Breeze can’t mint the session token, Start says why (sign in again / can’t reach TypeBuild / access changed) and no tab opens · when the session ends while you still hold the claim, a prompt offers Release · PHI-safe — the tab is labelled generically (“TypeBuild task …”) and terminal scrollback is never written to disk' },
