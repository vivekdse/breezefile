...
const SLIDES: Slide[] = [
  ...
  },
  {
    ...
    verbs: [
      ...
      { name: 'term', what: 'open an embedded terminal pane rooted at this folder · :term-close to dismiss · drop files (from Finder, web pages, or Breeze rows) onto the pane to paste their absolute paths into the running shell / Claude Code prompt' },
      { name: 'claude / codex / gemini', what: 'open the terminal pane and launch the AI CLI · when the launcher has variants the verb gains a Flags slot — Space toggles each flag (e.g. Continue, Skip-permissions), Enter launches with the union (no flags = bare) · backgrounded tabs badge red when Claude is waiting for you (turn end OR mid-turn permission prompt) · dock badge + system notification when Breeze is in the background (toggles in Settings → Notifications)' },
      ...
      { name: 'tasks', what: 'open the singleton Tasks tab — group by folder/status/due, search, filter, derived views (overdue / this week / scheduled / orphaned), checkbox + arrow-key selection, all bulk actions via verbs (no letter shortcuts)' },
      ...
      { name: 'tasks-tab keys', what: '↑/↓ move cursor · Shift+↑/↓ extend selection · Space toggle select on cursor row · Enter open edit · / focus search · everything else stays in the chip prompt' },
      { name: 'tasks-tab row buttons', what: 'every row carries: ✓ mark done (or ↺ reopen when closed) · ✎ edit · ↗ open in task tab · ⋮ more (status submenu, set due quick presets, pin, go to folder, delete) · the status pill itself is clickable to cycle pending → in_progress → done → cancelled' },
