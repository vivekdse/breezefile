...
const SLIDES: Slide[] = [
  ...
  },
  {
    ...
    verbs: [
      ...
      { name: 'new-home', chord: ':new-home / :nh', what: 'agent work monitor for a project — Hero Stats (Done / In Progress / Needs You / Failed) double as filter toggles, the Roster table lists every task with who acted last (🤖 / 👤 / 🤖+👤 — derived from the audit trail when available), and one-click Answer/Retry, with a Search box + status pills to filter it — focus a row and use j/k or ↑/↓ to move between rows and Enter to open the detail dialog (scoped to the table, so it never steals j/k from the rest of the app) — the box takes plain words OR a SQL-like query over task fields (e.g. `status in (needs, failed)`, `due < now+7d`), the same no-eval DSL the tag filters use; the copilot drives it via set_roster_filter (status/text) and query_roster (structured), and an Outcomes rollup below groups finished work (Done vs Failed) with a one-line result per task · click any row to open the detail dialog: full evidence log, structured outcome, cancel/retry, and a talk-back box to message the agent · a project carries no separate configuration — it is just a category + this derived view · + New Task opens the canonical Task form (the same one the task verb / Sidebar / copilot create_task open) and pops the copilot chat alongside it, so you can fill the form by hand or drive it conversationally' },
