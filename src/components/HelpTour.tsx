...
const SLIDES: Slide[] = [
  ...
  },
  {
    ...
    verbs: [
      ...
      { name: 'browser tool repository', what: 'a Playwright task drives a live side-by-side browser window AND consults a reusable-tool repository at ~/.breezefile/tools/ before writing one-off automation · the agent runs `breeze-tools available <url>` to find a matching tool, `help <id>` for its params, `run <id> --p v` to execute it (structured JSON + a 0–8 exit code: 0 ok · 4 auth · 5 page-changed · 7 precondition), falling back to raw page-driving only when no tool fits · ships with seed tools (gmail-prefill-send, web-form-login, extract-table); drop a `<id>/tool.json` + `tool.mjs` folder in to add your own · per-tool runs.jsonl tracks success rate · the operator session is one window split into two panes — the browser page on the LEFT, the Claude-Code terminal on the RIGHT, with ONE divider you drag to resize both · the — minimize button collapses the Claude pane (1/3 ↔ 0) and ✦ Claude re-shows it; the divider position + collapsed state persist across sessions · the ✕ close button ends the WHOLE session at once (it tears down the browser window AND the Claude PTY together, then offers to release the task if you still hold the claim)' },
