...
const SLIDES: Slide[] = [
  ...
  },
  {
    ...
    verbs: [
      ...
      { name: 'remote sources', what: 'sign in to TypeBuild (Settings → TypeBuild) — one “Sign in” button opens your browser to the TypeBuild page, where you sign in with Google or email & password; the session comes back into the app automatically (an email & password fallback lives behind a disclosure) — and its tasks appear in the same list, grouped by source · they’re read-only here (no edit/delete) and PHI-safe — titles/bodies stay in memory, never written to disk · row buttons gain Claim (when pending + unclaimed), Release (when you hold it), and Reopen (when the source marks it blocked); a failed/partial/blocked source status shows as a badge next to the title · claiming a task someone else grabbed shows an inline “couldn’t claim” notice' },
      { name: 'Start a TypeBuild task', what: '▸ Start on a pending TypeBuild row launches an interactive Claude session in a new tab, pre-wired to the task — you never type a command · the session starts already authenticated (no /mcp sign-in prompt) and the agent claims the task in-session · enabled only when you’re signed in and the onboarding prerequisites pass (Settings → TypeBuild: Claude Code + Chrome); the disabled tooltip says what’s missing · if Breeze can’t mint the session token, Start says why (sign in again / can’t reach TypeBuild / access changed) and no tab opens · when the session ends while you still hold the claim, a prompt offers Release · the secure session token lasts ~8h and can’t refresh mid-session — Breeze warns on the tab before it expires and, at expiry, offers a one-click “Restart task” that resumes the same conversation with a fresh token (you never see a raw connection error) · PHI-safe — the tab is labelled generically (“TypeBuild task …”) and terminal scrollback is never written to disk' },
