...
const SLIDES: Slide[] = [
  ...
  },
  {
    ...
    verbs: [
      ...
      { name: 'project task', what: 'creating a task scoped to a project now uses the SAME composer as everything else — there is no separate flow. From a project in the Projects tab (drill into it with l/Enter), press n (or the ＋ New task button) to open the composer with that project PRE-SELECTED; the project rides the create and carries its folder + cascading instructions as agent context. Opening + New task from a folder that belongs to a project also auto-attaches that project. Either way you can override the project or pick None.' },
      ...
      { name: 'projects', what: 'open the singleton Projects tab (aliases atlas / project home) — now an email-INBOX: one project per LINE, ranked by WHAT NEEDS YOU, not raw recency, in the file-manager folder aesthetic (a project reads like a FOLDER — a status glyph, the NAME, a short DESCRIPTION). Minimal status-icon vocabulary: ⛔ blocked · ⚑ needs you · ◷ working · ◦ idle · ◌ clear · ▸ has sub-projects. Rows wanting a human/agent (open/unclaimed · blocked · overdue · failed) lead the list, most-needing-attention first; each shows an amber “⚑ N” count plus a per-project attention summary like “3 open · 1 blocked · 1 overdue” (non-zero counts only). Projects that need nothing right now sit below a “nothing needs you below” fold; a project with no attention AND no activity in >7 days is HIDDEN — a “Show all projects (N hidden)” toggle reveals them dimmed, and the toggle is remembered. (TypeBuild’s list endpoint carries no real per-task timestamps yet, so idle-hiding only kicks in for sources that do.) Stats + attention roll UP from sub-projects; a project with children shows “· N sub-projects” and drilling in re-scopes the SAME list (with a folder-style breadcrumb back: Projects › … ). Drill into a project → L2: its parent→child task tree with roll-up sentences on parents (“4 of 6 done · 1 needs you”), blocked-by dependencies inline, one status badge per row · Enter on a leaf opens that task in the Tasks tab. Keys: j/k move · gg / G jump to top / bottom · l/Enter drill in (or expand a parent) · h/Esc back one level · / open the UNIFIED quick-switcher · : for the palette' },
      { name: 'project search (/)', what: 'on the Projects tab press / to open the UNIFIED quick-switcher over BOTH projects AND tasks: type to filter by name/description (projects) or title (tasks), grouped Projects / Tasks · ↑/↓ (or ⌃n/⌃p) move · ↵ opens — a project drills into it, a task NAVIGATES to the Tasks tab focused on that row (it does NOT fork or start the task) · esc closes · each result shows the same status glyph and its folder-style project path' },
      { name: 'archive a project', what: 'each project row in the inbox carries an archive action (⊟) on hover — and an ⊟ Archive / ↺ Unarchive button in the project’s detail header — that moves it out of the way without deleting it. Archived projects are HIDDEN from the list by default (the server omits them); a “Show archived” toggle below the list reveals them (the row reads struck-through with an “archived” tag and the action flips to ↺ Unarchive). The toggle is remembered across sessions.' },
  ...
  },
  {
    ...
    verbs: [
      ...
      { name: ':secrets', what: 'manage your saved credentials (NPI, Tax ID, login IDs) that the agent fills into forms — aliases vault/credentials · masked list, reveal-on-click, create, delete · write-only secret fields (SSN, DOB, bank account) are shown but cannot be revealed (the server refuses to read them back) · server-backed, no plaintext stored on this machine' },
