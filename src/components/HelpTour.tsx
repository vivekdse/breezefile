...
const SLIDES: Slide[] = [
  ...
  },
  {
    ...
    verbs: [
      ...
      { name: 'composer keys', what: 'inside the New/Edit task composer: ↑/↓ walk the questions, digits pick an option, ↵ pick+advance, ⌘↵ create at any time · the form is split into an always-visible MAIN part (title · project · who runs it · input/output fields · outputs · make this a template · body) and a collapsible ADVANCED OPTIONS section (collapsed by default) — press A (or click the header) to expand/collapse it, and press T anywhere to toggle “Make this a template” (see its own catalog row); it holds priority, defer/start, which agent, status, launch flags, due/schedule, pin and (local target) the working folder, and its header reads “Advanced options (N set)” when collapsed, where N counts the fields you left non-default · when collapsed, ↓/↵ off the last main question goes straight to create, never into the hidden advanced questions · there is no longer a “Task vs Chained task” chooser inside the composer — chaining has its own dedicated entry button · Inputs and Outputs are two separate, explained steps in the SAME question walk (no nested cursor): i adds an input · o adds an output, from anywhere in the Inputs/Outputs region · adding a field (or pressing ↵ on an already-defined one to edit it) opens a per-field SUB-WALK, one question per step, in the SAME ↑/↓/↵/digit grammar as the rest of the form — key → label → type (digits 1–5 pick text/number/date/select/bool) → options (select only) → required (outputs only, yes/no) · ↵ accepts the step and advances (commits the field on the last step) · ↑ steps back (exits the sub-walk on the first step) · Esc cancels the in-progress field · ⌘/Ctrl+⌫ on an existing field removes it · adding an input first offers a source picker — Custom is option 1, then fields exposed by your approved external APIs · the focused field shows a clear label at its top-left (Who runs this · Project · Folder · Notes · …) and scrolls to the top of the view when you activate it, so its options are in sight without scrolling · “Who runs this?” is now the UNION of every human in your TypeBuild groups (labelled by display name or email) PLUS Claude Code — picking a person makes a MANUAL task assigned to them (assigned_to = their email, the launch-flags block stays hidden), picking Claude Code is the agent/auto path (the Advanced launch-flags block returns); on the local target (or before members load) it falls back to the plain Manual + Claude Code pair · start date defaults to today on new tasks · the due question shows quick-pick chips — 1 week (W) · this Friday (F) · Monday (M) — for manual tasks · in the notes field plain ↵ adds a newline and ⌘↵ jumps to the next field instead of creating · for a TypeBuild task the second question is Project — the task’s CURRENT project comes first and focused (↵ confirms it), then your top 3 busiest other projects, then None, then “Other…” which opens a type-ahead over every project (↑↓/↵, Esc back); digits pick the short list. Opening + New task from a folder that belongs to a project AUTO-ATTACHES that project — it shows as a chip (“auto-attached from <folder>”) so it’s visible, not magic, and you can override or pick None; the chosen project rides the create and editing a TypeBuild task can change it too · after Priority a TypeBuild task offers an AGENT picker — assign the task to ONE agent (or None): each option shows the agent’s name + its launch mode (chrome / auto / resume / manual); ↑/↓ + ↵ or digits pick, None clears the assignment · agents with no group still list; the chosen agent rides the create and editing a TypeBuild task can change it too. The assigned agent (name + launch mode) then shows as a read-only Agent line in the task’s detail panel' },
      { name: 'input from API field', what: 'pressing i (or the “+ input” button) on the composer’s Inputs step — and the “+ Add input” button in the Template editor — opens a SOURCE-AWARE picker as the first sub-walk question: “Custom” is always option 1, then one option per field exposed by your approved external APIs (SavedQueries) — e.g. “people-search → name”, “people-search → dob”. Picking an API field carries you into the rest of the sub-walk (key/label prefilled — the field name, normalized + deduped — still editable) already BOUND to that query — the row shows a small ↪ badge with the query name and an ✕ to unbind it (renaming the key never touches the binding). At fill time a bound input becomes a live typeahead against that API. Picking “Custom” walks the plain key/label/type sub-walk, exactly like an output. If you’re signed out / offline / have no approved queries, only “Custom” shows — the walk stays fully usable.' },
      { name: 'input from a Connection (whole-record snapshot)', what: 'an input field can also bind a registered Connection’s live lookup (see the Connections verb) instead of a SavedQuery — same typeahead widget, but the search runs CLIENT-DIRECT against the external API (never through the TypeBuild server) and picking a row snapshots the WHOLE record, not just the display label: every field the lookup declares is written into the task’s data bag as `<key>.<field>` sibling entries, alongside `<key>` (the display label) and `<key>.ref` (the opaque record pointer) — plus three provenance keys, `<key>.connection_id`, `<key>.connection_version` (the Connection’s spec version this binding was authored against), and `<key>.picked_at` (when you picked it), so you can always trace exactly which Connection/version/moment produced a value. Picking a DIFFERENT record — or clearing the field — removes every stale `<key>.*` entry from the prior pick first, so nothing orphaned lingers in the data bag.' },
      ...
      { name: 'one primary action', what: 'every row shows exactly ONE primary button for what to do next — ✓ Done (manual) · ↺ Reopen · ▸ Start (TypeBuild) · ▸ Run now (auto) · ⧉ Open session (when a live session tab exists) · ◷ View run · ■ Stop (a running TypeBuild task — ends its session and releases the claim within seconds, taking priority over every other row action while a session is plausibly live) — everything else (pin, edit, open tab, status, due presets, go to folder, release, mark complete, cancel, reopen, delete) lives in the ⋮ kebab · a task someone else holds shows ◆ claimed by {who} (hover for who + how long ago, tinted when the claim is near expiry) and no action' },
      ...
      { name: 'is this view current?', what: 'Home reconciles with TypeBuild every 30s, and says so when it hasn’t: a quiet banner (“Last synced 2h ago — this view may be out of date”) appears above the topbar once a sync is overdue, so a frozen roster is never mistaken for a known-good empty one · it carries a Sync now button that forces an immediate pull rather than leaving you stuck — on success the banner clears itself; if the pull genuinely fails it says “Sync failed” and stays up, because the view really is still stale · a SEPARATE banner (“TypeBuild is responding slowly”) means the server is slow but your cached work is intact — that one clears on its own once the server recovers' },
    ],
  },
  {
    kind: 'catalog',
    ...
    verbs: [
      ...
      { name: 'agent flags', what: 'on a Claude task the composer offers flags — now a MULTI-SELECT question inside the ADVANCED OPTIONS section (press A to expand): Interactive (run opens in a new tab with an embedded claude session you converse and approve in, instead of headless), Chrome (let claude drive a browser, --chrome), Playwright (drive a side-by-side Breeze browser window over CDP — see “browser tool repository” below), Auto-accept (permissive edits, still human-gated) · walk to the Launch flags question and toggle each flag on/off with its digit (1–4) or ↵; ↓ off the last flag advances · each ON flag counts toward the “Advanced options (N set)” badge · shown only for Claude tasks (hidden when Manual) · an interactive cron fire opens a tab and pings you at the approval gate when the GUI is running; headless breezed falls back to a headless run' },
      { name: 'attach an agent to your browser', chord: '⌘B / Ctrl+B', what: 'opens the embedded browser WITH a live agent attached — the same operator arrangement a browser task gets: the page on the LEFT, the agent’s terminal on the RIGHT, driven over CDP so the agent can actually navigate, click, fill, and read the page for you · currently spawns the Pi coding agent by default (testing Pi as an alternate operator agent; Pi uses your own ~/.pi provider login, reaches the TypeBuild tools through its MCP adapter, and — unlike Claude Code — has NO permission prompts, so its shell/tools run ungated) · no task is involved, so the agent starts with GENERIC “you are attached to your browser — drive it as I ask” instructions and nothing PHI · just tell it what you want done in the terminal pane · it runs in the app-owned workspace (~/.breezefile/tasks/) with the standing browser playbook loaded, exactly like a task session · press ⌘B again to jump back to the pair rather than starting a second one; closing the window ends the WHOLE session (browser + agent PTY together), no orphaned session left behind' },
      { name: 'browser tool repository', what: 'a Playwright task drives a live side-by-side browser window AND consults a reusable-tool repository at ~/.breezefile/tools/ before writing one-off automation · the agent runs `breeze-tools available <url>` to find a matching tool, `help <id>` for its params, `run <id> --p v` to execute it (structured JSON + a 0–8 exit code: 0 ok · 4 auth · 5 page-changed · 7 precondition), falling back to raw page-driving only when no tool fits · ships with seed tools (gmail-prefill-send, web-form-login, extract-table); drop a `<id>/tool.json` + `tool.mjs` folder in to add your own · per-tool runs.jsonl tracks success rate · THE API SHORTCUT: the agent can read/replay the page’s own XHR/fetch and skip the rendered UI entirely — `net-observe [urlFilter]` watches the page’s requests (NON-PHI metadata: method/url/status, never bodies) to find which one carries the data, then `net-replay <url> [--method M]` re-issues it through your signed-in session (no clicks, no re-auth); a GET is a safe read, a mutating POST/PUT stays human-gated (refused without --allow-mutation) · SITE API RECALL: a discovered endpoint is remembered as a keys-only `api-spec` note keyed by domain (NON-PHI: method/path/header+param KEY names/me.* auth ref, never a value) — a successful `net-replay` auto-records it, `breeze-tools available <url>` auto-recalls it (surfaced as `api_specs` with `prefer_api:true` so the next task net-replays instead of re-driving), and `breeze-tools api-spec recall <url>` / `api-spec record --url … [--header n] [--param k] [--auth me.key]` are the first-class verbs · AUTO-PROMOTION: after the slow full-agent path solves a NEW page it auto-emits a reusable step-structured tool — `breeze-tools promote-from <id> --match <url> --actions <captured.json>` (or `--recording`) scaffolds a `status:candidate` tool (param/KEY refs only, never a value) that syncs to every runner and promotes itself to `active` after it passes a run or two; “support a new platform” = “run it once, keep the tool it leaves behind” · the agent also keeps durable NON-PHI memory via `breeze-tools memory` — `--site` notes (selectors, fast paths, gotchas for a domain) are now SHARED ONLINE, so a learning on one machine helps every machine + teammate, with a local cache for offline; `--task` notes stay local to this machine · the standing browser playbook (the operator instructions every session runs by) is now fetched from the server at session start — edit it once and every machine picks it up, with the bundled default + a cached copy as the offline fallback · ON-DEMAND ESCALATION: a task that started TERMINAL-only (no Chrome/Playwright flag, so no browser pane) can bring its browser up MID-RUN — the agent runs `escalate [url]` (alias `use-browser`) and the operator window opens bound to its OWN session, terminal beside it, with no takeover prompt against itself; it never needs to be relaunched to gain the browser · the operator session is one window split into two panes — the browser page on the LEFT, the Claude-Code terminal on the RIGHT, with ONE divider you drag to resize both · before the agent’s first navigation the browser pane shows a calm “Starting your task…” splash (styled in your current theme) instead of a blank page, replaced the moment the agent opens its first real page · the — minimize button collapses the Claude pane (1/3 ↔ 0) and ✦ Claude re-shows it; the divider position + collapsed state persist across sessions · the ✕ close button ends the WHOLE session at once (it tears down the browser window AND the Claude PTY together, then offers to release the task if you still hold the claim) · NO-BROWSER CHANNEL: a tool.json can also declare `channel:"mcp"` with a `service_url` — its steps call a first-party MCP catalog server (e.g. `connectors`, "Connected Services": Gmail/Slack/etc via the user’s already-authorized accounts) directly over HTTP via `ctx.mcpCall(tool, args)`, using the session’s own TYPEBUILD_MCP_TOKEN, so no CDP connection is ever opened; the seeded `connectors-call` tool is the generic wrapper (`run connectors-call --op list_catalog|list_connections|connect|poll|disconnect|call|execute|list_actions|describe_action`) — `execute` is the preferred "do something" path (runs backend-side with the vaulted token; works with secret-masking ON), `list_actions`/`describe_action` discover a service\'s actions and their arg schemas, and raw `call` now needs masking OFF — promote a narrower tool once a specific call shape proves durable, same discipline as a browser-channel promotion · FIELD PERCEPTION + ACTION (the agent’s JAWS-style form reader/writer): `page` gives a one-screen orientation skeleton, `fields` lists every form field (ref · kind · label · value · flags, descending into iframes) and mints a ref map, `field <ref>` prints one field’s full contract as JSON (kind, options, howToSet) — then the ref-based SETTERS commit by that stable ref: `field-fill <ref> <value>` (or `--data-ref <key>` for a PHI value resolved in-process, never on argv/stdout) for text/date, and `field-select <ref> --pick <label> [--query <text> | --query-ref <key>]` which dispatches per species (native select · static combobox · async autocomplete that waits for “N results available.” · virtualized listbox by arrow-walk) and matches --pick exact → normalized → unambiguous substring, printing the nearest candidates + a retry line rather than ever guessing · SELF-DOCUMENTING DRIVER: the driver CLI is its own manual — `help` prints the compact verb table (grouped cheapest-first, field layer before raw selectors), `help <verb>` prints that one verb’s usage + a runnable example, and a mistyped verb returns a nearest-match suggestion + `try: help` instead of a bare error (both run without a browser connection, so a confused session can always orient) · when a raw click/fill/type is stuck in a retry streak the run-metrics hint now also points UP a level — “try `fields`” — so the agent enumerates what’s actionable by ref instead of guessing selectors' },
      { name: 'the Brain: tiered shared knowledge', what: 'the agent\'s NON-PHI notes and tools are curated across THREE isolation tiers — Global (shared across every business on the network), Org (your organization only), Task (this run only) — each note in the site-notes surface shows a small tier pill plus a confidence badge (high/medium/low) blended from the brain\'s hit-rate, downstream-success-rate, and staleness signals, so you can see at a glance how trustworthy a piece of shared know-how is · the agent can also pull a previously-generalized TOOL artifact straight from the brain by id or signature instead of rediscovering it · new candidate knowledge is AUTO-CONFIRMED by default for agent-driven flows (nothing blocks on a human) — an async curator promotes candidates to active on its own schedule; a confirm/reject card only appears for the rarer case a specific candidate is surfaced for review, and Reject just stops that card from reappearing (it can\'t delete anything already recorded, since promotion is curator-only) · the offline cache only ever mirrors ACTIVE knowledge, never an unreviewed candidate' },
      ...
      { name: 'browser shortcuts', chord: '⌘L / ⌘R / ⌥← ⌥→', what: 'in any embedded browser surface (an in-app tab OR the operator session pane), Chromium-style keyboard shortcuts drive the toolbar: ⌘L (Ctrl+L on Linux/Windows) focuses + selects the address bar, ⌘R reloads the page, ⌥← / ⌥→ (or ⌘[ / ⌘]) go back / forward · they fire only while focus is inside a browser surface — they never hijack keys elsewhere in the app · new-tab (⌘T) and bookmark (⌘D) are intentionally absent: this surface reuses a single browser tab and has no bookmark action' },
  ...
  },
  {
    ...
    verbs: [
      ...
      { name: 'connections', chord: ':connections / :register', what: 'connect the external services your admin has provisioned — the panel leads with "Available connections" tiles (QuickBooks, a scheduling API, an MCP server, …): click Connect, finish the OAuth sign-in in your browser, done — no credential is ever typed or shown; the token lives in the TypeBuild server vault and data still flows client-direct once a connection is wired to a task. "Your connections" below lists what\'s registered (kind REST/MCP, endpoint, scope, credential-set state) with edit/delete inline, and an Advanced disclosure retains manual registration (endpoint + pasted credential, masked-by-default) for the rare service you hold the credential for yourself' },
      { name: 'new-home', chord: ':new-home / :nh', what: 'agent work monitor for a project — Hero Stats (Needs You / Open / In Progress / Scheduled / Failed / Done, in that order) ARE the status filter (click a card to filter, click again to clear — the old duplicate pill bar is gone); the cards are WEIGHTED by what wants you: a bucket that needs a human and actually has work in it (Needs You / Open / Failed) shows big in its status color, while Done — and any empty card — stays deliberately muted, because a pile of finished work is a record, not a call to action. SCHEDULED vs OPEN is a real distinction, not two words for waiting: SCHEDULED means something will genuinely get to it (the task carries a cron / next-run time), OPEN means nothing is coming and it wants a human. A due date alone is a deadline, not a schedule — those read Open; the active card LIGHTS UP in its own status color — a soft fill, a full-strength ring, and its label/count pulled into the accent — so the applied filter is unmistakable, and ONE UNIFIED Roster table lists every kind of work with the SAME columns — Title · Status · Last Run · Who · Runs · Actions: a template group, a chained task, and a plain task are all just rows (a row that aggregates several runs/steps shows a per-bucket STATUS BREAKDOWN — “3 done · 2 open” — and its Runs count; a plain task shows one pill and Runs = 1; a finished plain task shows its ONE-LINE OUTCOME right under the title — the old separate Outcomes panel is folded into the table) · who acted last is 🤖 / 👤 / 🤖+👤 (derived from the audit trail when available) · every row keeps its one-click primary action — Answer / Retry / ▶ Start (Start claims + launches a Claude session for an open, unclaimed task — the same play-button mechanism as the Tasks page) / ■ Stop (a running task — ends its session and releases the claim within seconds; takes priority over every other action whenever a session is plausibly live) or View → — PLUS a ⋮ actions menu carrying the row’s full action list (View · ▶ Run all · + New run · Auto-continue · ↗ Open task, as applicable for a template group; a plain task’s ⋮ menu offers ↗ Open task plus ▶ Run and Cancel when applicable), with a Search box to narrow the table · the roster leads with the MOST RECENT activity and hides tasks completed more than two weeks ago so Home shows live work, not a month of history — a “Show N older tasks” link at the bottom reveals them (filtering to Done, or searching, always includes them); a long roster paints a bounded first page with a “Load more” button rather than every task at once — focus a row and use j/k or ↑/↓ to move between rows and Enter to open the detail dialog (scoped to the table, so it never steals j/k from the rest of the app) — the box takes plain words OR a SQL-like query over task fields (e.g. `status in (needs, failed)`, `due < now+7d`), the same no-eval DSL the tag filters use; the copilot drives it via set_roster_filter (status/text) and query_roster (structured); whenever a status bucket and/or a search/query is active a Filtering chip bar sits above the roster showing each active filter in human-readable form (a ⚡ marks a structured query, ⚠ an invalid one) — click a chip’s ✕ to clear just that filter or Clear all to reset the roster, so what the copilot filters is always visible and clearable by hand · GROUP is the primary scope: a GROUP picker sits FIRST in the topbar (shown when you belong to any group), listing your groups by NAME with a task count; the PROJECT picker to its right then narrows to the SELECTED group’s projects only (groups-first — projects nest under the chosen group), and switching to a group that doesn’t contain your current project clears the project so the two stay coherent. "All groups" is the default (no group scope · all projects); picking a group scopes the roster, the Hero Stats, and the subproject sections to it, shows a "Group: <name>" chip in the Filtering bar (clear it with its ✕), and is remembered across sessions like the project pick · click any plain row to open the detail dialog: full evidence log, structured outcome, cancel / retry / ▶ start, and a talk-back box to message the agent — a DONE task opens straight into its outcome/result view (not the edit form), and a completed single (non-chain) task that carries a fielded result gets its own View → action in the row so the result is always one click away, the same as a finished chain’s View → into its run matrix · TASKS THAT SHARE A TEMPLATE collapse into ONE summary row (two-level view): Level 1 shows the template name, the NUMBER OF RUNS, a STATUS BREAKDOWN (how many done / in progress / scheduled / open / needs-you / failed, one colored chip each), and WHO they’re ASSIGNED TO (a single name, or “N people” with a tooltip listing everyone — a chain’s steps can be spread across several assignees), plus View → as the primary action and a ⋮ menu with View runs · ▶ Run all (starts every runnable run in the group) · + New run — no jagged per-template columns at this level, so stacked groups stay aligned · click the row (or View →) to drop into Level 2, the run MATRIX: one ROW PER RUN, COLUMNS are that run’s INPUTS then OUTPUTS (grouped by step ①②③ for a chain, or the task’s own input/output fields for a simple template), a STATUS chip per row, and per-cell hover actions (✎ Enter value on inputs · ▶ Start step / ▶ Run · ↗ Open run); ← Back returns to the roster (output-cell editing is coming once the server result-write lands) · for a single template the matrix header has an ✎ Edit template button that opens the template DEFINITION editor — rename, add/remove/edit its input + output fields, and edit the prompt/notes body — saved to the template itself (owner-only); editing does NOT change runs already created from it (they keep their own values), only new runs pick up the change (chain-template editing lands once the server chain-update endpoint ships) · a project carries no separate configuration — it is just a category + this derived view · + New Task opens the canonical Task form (the same one the task verb / Sidebar / copilot create_task open) and pops the copilot chat alongside it, so you can fill the form by hand or drive it conversationally · SUB-PROJECTS roll up into the roster: pick a parent project (or "All projects") and its ENTIRE subproject subtree is aggregated — the Hero Stats count every descendant task, and each direct child subproject appears as a navigable SECTION ROW (a 🗂 name, a per-bucket status breakdown, and its task count) so a parent with no direct tasks of its own is never empty; click a subproject row (or its Open →) to drill into it, and use the breadcrumb above the hero (All projects › parent › current) to climb back out' },
      { name: 'groups', chord: ':groups / :teams', what: 'the team/group management surface — a singleton tab (also reachable via the "Manage groups" link beside New Home\'s group picker). MASTER-DETAIL: the LEFT rail lists every group you belong to (a shield glyph, name, your role badge — ★ admin / ● member — a member count, and a pending-invite count when any); above it a "Pending invites (N)" section appears whenever someone has invited YOU, each with Accept / Decline. A "+ New group" input at the bottom creates one and selects it. The RIGHT pane shows the selected group: its name (admins get inline Rename + a guarded Delete that warns projects/tasks are re-homed and offers a target-group picker — you can never delete your only group), a MEMBERS table (initial avatar, principal, display name, role badge, a "(you)" marker on yourself), an "Invited" subsection for pending members (muted, "invited by X"), a "Projects" section listing the group\'s projects, and — for admins — a "+ Invite" control (email + member/admin role → sends an invite) plus per-row remove (×) and promote/demote toggles. Non-admins get a read-only roster. Role changes hide themselves automatically if the server route isn\'t deployed yet.' },
      { name: 'chained tasks (a chain of templates)', what: 'a CHAIN is a HIGHER-ORDER TEMPLATE: nothing but an ORDERED LIST OF SAVED TEMPLATES. The "New Chained Task" entry opens a builder that is just a template PICKER + ordering — there is NO inline field/instruction editing (the templates you pick already own their input/output fields). "+ add template" opens a typeahead over your saved templates (↑/↓ highlight · Enter adds · Esc closes — the SAME template list the "New from Template" picker uses); each chosen row shows the template name + its input/output field counts, with ↑/↓/✕ buttons to reorder or drop it · you need at least one template to continue · if you have no saved templates yet the builder tells you to create some first (via "Make this a template" on a task) · the chain’s own step is a thin container: just its title + project, no who/notes/advanced questions' },
      { name: 'structure these fields?', what: 'writing input/output intent as plain PROSE in the notes step (e.g. "Input: Site name (or URL)" / "Output: First headline from the site", or "needs a customer email" / "produces a summary") is detected live — a one-tap "Structure these fields?" banner appears under notes, showing exactly what it parsed · Accept adds them as real input/output field DEFINITIONS on the task (same fields step, same editors — nothing is auto-submitted); "Keep as text" dismisses it and the prose is untouched either way, so it is always a suggestion, never a silent rewrite · only offered for a fresh plain task with no fields defined yet · the copilot\'s create_task action runs the SAME parser on conversational requests, so "make a task that takes a site and outputs its first headline" opens the New Task form with those fields pre-filled instead of leaving it as unstructured prose the agent has nothing to act on' },
      { name: 'create a job (from a chained task)', what: 'submitting a "New Chained Task" spawns a JOB: a thin meta-parent container (the chain’s title + project) plus one CHILD task per picked template — each child is the template INSTANTIATED into a real task (inheriting its output schema, input fields/data-keys, project, agent and flags), linked to the parent and to its predecessor (parent + depends-on wired so the steps run in order) · creation does NOT ask for the input VALUES — each child instantiates with its fields defined but empty, and you fill them later inline in the roster / the drawer’s Inputs editor · a blank (non-chained) task is byte-identical to before' },
      { name: 'make this a template', chord: 'T', what: 'a MAIN-form yes/no step in the New/Edit task composer (right before Body) — press T anywhere in the composer to toggle it, or ↑/↓ to the step and pick 1/2 · a TEMPLATE is a reusable task definition: this task’s title plus its declared INPUT & OUTPUT fields, which become the template’s VARIABLES · later you pick “New from Template” (see “from a template”) and only fill the values — project, output schema, agent and flags all inherit · you do NOT save a template separately: the server AUTO-registers (and de-dupes) one from any task created with input fields, so this toggle is intent + a guarantee — turning it ON requires the task to have at least one input field on save (else it bounces you back to the Inputs & outputs step to add a variable) · in EDIT mode the toggle REFLECTS whether the task already backs a template (matched by name) and reads-only that state · only shown for the TypeBuild target (not local, not a chained job)' },
      { name: 'from a template', chord: ':new-from-template / :nft', what: 'a FIRST-CLASS entry separate from plain create: the "+ New from Template" button (New Home topbar) or :new-from-template / :nft opens a searchable keyboard picker over your TEMPLATES · templates are now saved AUTOMATICALLY — whenever you create a task that declares input fields, the server registers (and de-dupes) a reusable template from it, so there is no separate "save template" step · the picker LISTS those templates (your project’s own plus shared/global ones, typeahead on name, each row showing its field count) · pick one, accept the prefilled TITLE (Enter), then answer ONE question per input VARIABLE — project, output schema, agent, and flags all INHERIT SILENTLY from the template, never asked · Ctrl+Enter creates the task server-side (data bag from your answers, output schema copied) — under 10 seconds end to end · a template with NO inputs (e.g. an output-only "Get top 5 headlines") is a one-click create: title → Enter, no value questions · the picker ALSO lists your saved CHAINS (multi-step workflows) alongside single templates — a chain row is marked with a small "Chain" badge and shows its STEP count instead of a field count; picking one INSTANTIATES the whole chain immediately (no title/value questions — a chain has no per-run variables), spawning the meta-parent + one child per step just like "New Chained Task" · an "Edit details…" link drops into the full form if you need to change more than the values (single templates only) · NOTE: this picker creates ONE task from a single template, or the whole workflow from a chain; to BUILD a new chain use "New Chained Task", whose builder picks + orders your saved templates (see "chained tasks") · the copilot understands template-shaped requests too (e.g. "new headline check for cnn.com" matches a template by name and asks just for the URL)' },
      { name: 'chained rows in the roster', what: 'a CHAINED task (a v2-chained job, or a thin parent container with step children) renders as ONE summary row in the same unified table — its Status column shows the per-step breakdown (“3 done · 2 open”), Runs counts its steps, and the row click / View → opens the run MATRIX (Level 2), where each step’s IN cells are editable inline and OUT cells are read-only, a needed-when-unmet step hatches n/a, and every cell’s hover menu offers ✎ Enter value · ▶ Start step · ↗ Open run · Auto-continue (start the next step automatically when the previous finishes) lives in the row’s ⋮ menu and keeps working even while you’re inside the matrix; an auto-start failure surfaces as an inline ⚠ reason on the row, never silently · bodies are fetched lazily and cached PHI-safely' },
      { name: 'outputs, evidence & pipeline rollup', what: 'the task detail dialog surfaces the chain loop: a CHILD task shows an OUTPUTS section (its declared output fields vs what the agent submitted, with an N-of-M required line) and its required outputs appear as first-class EVIDENCE entries in the log — an unmet requirement reads “awaiting agent — owes N required output(s)” · a META-PARENT (job) task shows a PIPELINE rollup: the ordered steps, each with a status pill (Done / In progress / Open / Not needed / Cancelled / Failed) and a one-line outcome, every row clicking through to its child task · the step pill MERGES the child task’s live server status: a CANCELLED step reads grey + struck-through (distinct from n/a “Not needed”) and is skipped by Start-chain / auto-continue; a FAILED/BLOCKED step reads red with a retry ▶ · every ▶ Start (row, step, or “Start chain”) always shows feedback — a “Starting…” pending state or an inline ⚠ reason (e.g. “no runnable step — remaining steps are cancelled”, “launch failed: …”) — it never silently does nothing' },
      { name: 'author a data-source query (chat)', what: 'in New Home, ask the copilot to build a data-source query in plain English (e.g. "a patient typeahead against the EHR") — grounded with the registered DataSources (name / base URL / entity types, NO credentials) and the sandbox code contract, it DRAFTS the query code + output schema (list_data_sources → draft_saved_query), then you TEST it for sample rows (test_saved_query) and APPROVE it on a mandatory review card that shows the exact code (approve_saved_query) · if the API isn\'t registered yet, ask it to REGISTER the data source first (e.g. "register the scheduling API as a data source") — register_data_source confirms the exact name / base URL / entity types (and stores any credential server-side, never echoed back) on a review card, mints the sourceId, and refreshes the grounding so you can author a query over it in the SAME conversation · approval is the design-time human gate AND the publish step — it freezes the version, lets it execute, and shares it with everyone in the project (drafts are private and never run; the executor refuses any non-approved version) · iterate with new_query_version (clone → new draft) · query code/schema are config (not PHI); sample rows are shown in chat memory-only' },
      { name: 'author custom form behavior (chat)', what: 'in New Home, ask the copilot to build custom FORM BEHAVIOR in plain English (e.g. "show a diagnosis typeahead only when the visit type is follow-up, and flag an age over 120") — grounded with the effect contract and the approved SavedQueries it can bind typeahead fields to, it DRAFTS extra fields[] + a PURE logic function (draft_form_extension), then you TEST it against sample values and see the returned effects (test_form_logic) and APPROVE it on a mandatory review card that shows the exact fields + logic (approve_form_extension) · the logic runs server-side and applies four allowlisted effects — set a value, show/hide a field, replace select options, or show an inline validation error — for a future client renderer to apply (the client never eval’s the logic and never injects markup) · iterate with new_form_extension_version (clone → new draft) · fields/logic are config (not PHI)' },
      ...
      { name: 'copilot', chord: '/', what: 'a persistent AI chat sidebar available on every surface — press / anywhere you\'re not typing into a field (or use the fm:openCopilotChat trigger some buttons fire) to open it; ask it to navigate the app (e.g. open New Home), create tasks or projects (including a chained task), filter the roster, or open a specific task · it can also ANSWER ANALYTICAL questions about your work by composing a query over your tasks/projects (run_query_plan) — grouping, counting, and aggregating that plain search can\'t, e.g. "group projects by repo", "which repo has the most failed tasks", "count tasks per assignee grouped by project" (read-only, metadata only — never task bodies) · it drives the same agent-work affordances you click on Home: answer an agent\'s pending question (answer_task, or bulk_answer_tasks to send one reply to many at once), retry or start a task (retry_task — the "Retry" button), and message the agent working a task (message_agent — the detail dialog\'s "Send message"); each of these shows an approve card before it sends · it can also EDIT a task\'s fields directly — retitle it, change priority, set/clear a due or defer date, move it to a different project, or reassign it (update_task; find the task first with find_tasks if it isn\'t already in view) — status changes go through set_task_status/cancel_task and pin/unpin through pin_task/unpin_task instead · needs an Anthropic API key configured (ANTHROPIC_API_KEY or the same key store the tag-assist NL box uses); shows a quiet setup hint instead when none is set, reachable the same way · chat text is not persisted to disk · the floating ✨ launcher button is hidden by default (it\'s a debug/inspect affordance, not the way in) — reveal it with Ctrl/Cmd+Shift+/ if you need it' },
    ],
