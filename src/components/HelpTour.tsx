...
const SLIDES: Slide[] = [
  ...
  },
  {
    ...
    verbs: [
      ...
      { name: 'add task / add project', what: 'creating things on Home is scoped to where you are. At the root, the header carries ＋ New task and ＋ New project. Drill INTO a project and its header bar carries ＋ New task (scoped to that project) and ＋ New sub-project (parented under it) — there is no per-row add button anymore. The n hotkey and the :new-task / :new-project verbs do the same scoped thing.' },
      { name: 'new-task', what: 'verb (aliases new task / add task / create task / mktask): creates a task scoped to the project in view on Home — the open project when you have drilled in, otherwise unscoped — opening the shared composer with that project pre-selected · same as the n hotkey · reachable from : and ⌘K.' },
