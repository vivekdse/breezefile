...
const SLIDES: Slide[] = [
  ...
  },
  {
    ...
    verbs: [
      ...
      { name: 'new-home', chord: ':new-home / :nh', what: 'agent work monitor for a project — an amber Approval Bar surfaces every task blocked on you first, Hero Stats (Done / In Progress / Needs You / Failed) double as filter toggles, the Roster table lists every task with per-project custom columns, who acted last (🤖 / 👤 / 🤖+👤), and one-click Answer/Retry, with a free-text Search box + status pills to filter it (the copilot can drive both via set_roster_filter), and an Outcomes rollup below groups finished work (Done vs Failed) with a one-line result per task · click any row to open the detail dialog: full evidence log, structured outcome, cancel/retry, and a talk-back box to message the agent · Customize opens an INLINE per-project template panel (right in the page, not a modal): custom fields, roster columns, Repeatable Tasks (reusable task templates you Run now or put on a Daily/Weekly/Monthly schedule), Chains (an ordered sequence of tasks — add a repeatable from the dropdown or a free-form step, reorder, then Run chain to spawn linked tasks), approval rules, and steps · every one of these is also drivable by the copilot, which shows the same panel as it edits · + New Task opens a conversational composer that turns chat answers into a structured task' },
