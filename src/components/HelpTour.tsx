...
const SLIDES: Slide[] = [
  ...
  },
  {
    ...
    verbs: [
      ...
      { name: 'from a template', chord: ':new-from-template / :nft', what: 'a FIRST-CLASS entry separate from plain create: the "+ New from Template" button (New Home topbar) or :new-from-template / :nft opens a searchable keyboard picker over prior tasks that carry field DEFINITIONS (fielded single tasks and chained-task parents, newest/most-used first, typeahead on title) · pick one, accept the prefilled TITLE (Enter), then answer ONE question per input VALUE (grouped by step for a chain) — project, notes, output schema, agent, flags, and priority all INHERIT SILENTLY from the picked template, never asked · Ctrl+Enter creates — under 10 seconds end to end · an "Edit details…" link drops into the full form if you need to change more than the values · a chained template instantiates parent+children the same way "create a job" does, just with your typed values already filled in · the copilot understands template-shaped requests too (e.g. "new headline check for cnn.com" matches a fielded template by title and asks just for the URL)' },
