...
const SLIDES: Slide[] = [
  ...
  },
  {
    ...
    verbs: [
      ...
      { name: 'attach an agent to your browser', chord: '⌘B / Ctrl+B', what: 'opens the embedded browser WITH a live Claude Code agent attached — the same operator arrangement a browser task gets: the page on the LEFT, the agent’s Claude-Code terminal on the RIGHT, driven over CDP so the agent can actually navigate, click, fill, and read the page for you · no task is involved, so the agent starts with GENERIC “you are attached to your browser — drive it as I ask” instructions and nothing PHI · just tell it what you want done in the terminal pane · it runs in the app-owned workspace (~/.breezefile/tasks/) with the browser tools pre-approved and the standing browser playbook loaded, exactly like a task session · press ⌘B again to jump back to the pair rather than starting a second one; closing the window ends the WHOLE session (browser + agent PTY together), no orphaned session left behind' },
