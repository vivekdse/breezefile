...
const SLIDES: Slide[] = [
  ...
  },
  {
    ...
    verbs: [
      ...
      { name: 'author a data-source query (chat)', what: 'in New Home, ask the copilot to build a data-source query in plain English (e.g. "a patient typeahead against the EHR") — grounded with the registered DataSources (name / base URL / entity types, NO credentials) and the sandbox code contract, it DRAFTS the query code + output schema (list_data_sources → draft_saved_query), then you TEST it for sample rows (test_saved_query) and APPROVE it on a mandatory review card that shows the exact code (approve_saved_query) · approval is the design-time human gate AND the publish step — it freezes the version, lets it execute, and shares it with everyone in the project (drafts are private and never run; the executor refuses any non-approved version) · iterate with new_query_version (clone → new draft) · query code/schema are config (not PHI); sample rows are shown in chat memory-only' },
