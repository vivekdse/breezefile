...
const SLIDES: Slide[] = [
  ...
  },
  {
    ...
    verbs: [
      ...
      { name: 'tasks', what: 'open the singleton Tasks tab — split layout (list + detail panel) organized by OWNER: FOR YOU (manual tasks you act on by hand) · FOR AGENTS (TypeBuild + auto-execute tasks) · DONE (collapsed, most-recent first) · one filter row: search, Show done, source dropdown (All / Breeze / TypeBuild), + New task · checkbox + arrow-key selection, all bulk actions via verbs' },
      { name: 'one primary action', what: 'every row shows exactly ONE primary button for what to do next — ✓ Done (manual) · ↺ Reopen · ▸ Start (TypeBuild) · ▸ Run now (auto) · ⧉ Open session (when a live session tab exists) · ◷ View run — everything else (pin, edit, open tab, status, due presets, go to folder, release, mark complete, delete) lives in the ⋮ kebab · a task someone else holds shows ◆ claimed by {who} and no action' },
      ...
      { name: 'detail panel', what: 'the right pane shows the cursor task: manual tasks get dates, status chips, and notes that COLLAPSE past ~8 lines with a Show more expand + inner scroll (long notes are finally readable) · TypeBuild tasks lazy-load their decrypted body on focus (held in memory only, never logged) plus a lifecycle block (status, claimed-by, attempts, priority) and Release / Reopen / Mark complete' },
      { name: 'remote sources', what: 'sign in to TypeBuild (Settings → TypeBuild) — one “Sign in” button opens your browser to the TypeBuild page, where you sign in with Google or email & password; the session comes back into the app automatically — and its tasks appear under FOR AGENTS · they’re read-only here (no edit/delete) and PHI-safe — titles/bodies stay in memory, never written to disk · a failed/partial/blocked source status shows as a badge next to the title · the ⋮ kebab carries Release (when you hold the claim) and Mark complete (server support is rolling out — if the endpoint isn’t live yet you’ll see “TypeBuild server doesn’t support completing tasks from here yet”)' },
