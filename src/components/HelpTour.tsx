...
const SLIDES: Slide[] = [
  ...
  },
  {
    ...
    verbs: [
      ...
      { name: 'sidebar indicators', what: 'Active Tasks sidebar shows per-task glyphs: due-now dot, running spinner, last-run-failed dot, ⚡ for auto-enabled · right-click a task for Edit / Mark done / Pin / Snooze / Run now / View run history / Copy resume command / Delete' },
      { name: 'run history', what: 'every auto run lands in a per-task history dialog: status, duration, attempt, conversation_id · Rerun button kicks off a fresh run via the same agent path · "Copy resume" puts `cd <folder> && claude --resume <id>` on the clipboard so you can drop into the headless trace from any terminal' },
