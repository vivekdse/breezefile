...
const SLIDES: Slide[] = [
  ...
  },
  {
    ...
    body: (
      <>
        ...
        <br /><br />
        <b>By hand</b> — keep a list of what you owe. Navigate Home like a
        folder (j/k/l/h), or open the flat list with <kbd>tasks</kbd>; mark
        them done as you go.
        <br /><br />
  ...
  },
  {
    ...
    verbs: [
      ...
      { name: 'project task', what: 'creating a task scoped to a project now uses the SAME composer as everything else — there is no separate flow. On Home, drill into a project (l/Enter) and press n (or the ＋ New task button in its folder header) to open the composer with that project PRE-SELECTED; the project rides the create and carries its folder + cascading instructions as agent context. Opening + New task from a folder that belongs to a project also auto-attaches that project. Either way you can override the project or pick None.' },
      ...
      { name: 'home', what: 'Home is the tasks-first landing — it OPENS FOCUSED when the app launches (the file manager is still there beside it, one tab-click / :files away; the file surface is now ONE ability, not the point). Aliases: home / tasks home / projects / atlas. Your work is shown as FOLDERS: each project reads exactly like a folder — a folder-style header (kicker · serif NAME · italic DESCRIPTION “◇ given to agents as context” · ⛓ bound folder · ❦ rule) with its TASKS listed beneath as file rows (PROJECT = FOLDER, TASK = FILE). Projects are attention-ranked: the ones needing a human/agent lead, with the hero line up top (“7 things need you, 2 blocked — Open X first →”) and an amber “⚑ N” count; projects that need nothing right now sit below a “nothing needs you below” fold, and idle ones hide behind “Show all projects (N hidden)”. Project-less tasks gather in a synthetic “Inbox (no project)” block, always first. At the root, sub-projects collapse to a single folder row (→); drill into a project to see it full, with sub-projects nesting as nested folder blocks. (TypeBuild’s list endpoint carries no real per-task timestamps yet, so “last activity” is only shown when the data backs it.) Click a project’s ⛓ folder to open its bound folder as a normal folder tab.' },
      { name: 'home keys', what: 'Home navigates like a FOLDER — one flat cursor walks project headers + task rows + sub-projects together: j/k (or ↓/↑) move · gg / G jump to first / last · l / Enter / → drill in (project header → open the project · sub-project → drill · task parent → expand · task leaf → open its detail drawer) · h / Esc / ← back one level · n (or ＋ New task) opens the composer pre-scoped to the project in view · Space selects a task row and : opens the palette to act on the selection (:done :reopen :in-progress :cancel :pin :unpin :delete run on the selected tasks via the same engine as the flat list) · / opens the unified quick-switcher · the breadcrumb shows the project ancestor chain (Home › Acme › Onboarding).' },
      { name: 'project search (/)', what: 'on Home press / to open the UNIFIED quick-switcher over BOTH projects AND tasks: type to filter by name/description (projects) or title (tasks), grouped Projects / Tasks · ↑/↓ (or ⌃n/⌃p) move · ↵ opens — a project drills into it, a task NAVIGATES to the Tasks tab focused on that row (it does NOT fork or start the task) · esc closes · each result shows the same status glyph and its folder-style project path' },
      { name: 'archive a project', what: 'each project folder block on Home carries an archive action (⊟) on hover — and an ⊟ Archive / ↺ Unarchive button in the project’s drilled-in header — that moves it out of the way without deleting it. Archived projects are HIDDEN by default (the server omits them); a “Show archived” toggle below the list reveals them (the header reads struck-through with an “archived” tag and the action flips to ↺ Unarchive). The toggle is remembered across sessions.' },
      { name: 'new-project', what: 'verb (aliases add project / create project / mkproject): open Home and expand the inline create form — capture a name, an agent-facing description sentence, an optional parent project (making it a sub-project), and an optional folder/repo binding → creates the project via TypeBuild · ⌘↵ submits, Esc cancels · also reachable as the ＋ New project / ＋ New sub-project button on Home' },
      ...
      { name: ':open-detail', what: 'verb (aliases inspect · detail · trace · session, or press Enter on a TypeBuild row / Open ↗ in the detail panel) — distinct from :open-task, which opens a TAB: opens the full task DIALOG — now FULL-SCREEN, segmented into Task details · Trace · Session (1/2/3 or h/l switch, Esc closes; task text is selectable/copyable) · TASK DETAILS is the FIRST tab and IS the task: it reuses the new-task composer form, pre-filled with the task’s current values (title, notes, who, when/due, defer, priority, project, status, pin) and fully EDITABLE — Save changes persists title, notes/body, AND routing-field edits (status · priority · due · defer · project) back via the TypeBuild PATCH (title + body are re-encrypted at rest; held in memory only, never logged); below the form sit the read-only schedule (recurring ↻ cron + next run), dependencies/parent-child, folder, and the cascading EFFECTIVE INSTRUCTION SET with provenance + a one-line summary (“8 — 4 project · 2 payer:HMO · 1 task”) plus a + Teach control that saves a correction to a SCOPE you pick: THE PROJECT persists into the project’s instructions (owner-only — a project you don’t own declines, and PHI-shaped text is refused) and THIS TASK persists as a per-task note (claim the task first; PHI-shaped text is refused); A CATEGORY is kept for the current session only until its server store ships (the picker says so) · TRACE is the live run timeline (steps of a running/completed run, the in-flight step pulses) · SESSION opens the live terminal session (in progress) or replays the last run · the header shows the live status (working / needs-you / blocked) with a ◼ Stop control (s) for a running task and an ↳ Enter thread control (e) to get into the agent thread when a task is in progress or waiting on you' },
