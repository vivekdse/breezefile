...
const SLIDES: Slide[] = [
  ...
  },
  {
    ...
    verbs: [
      ...
      { name: 'connections', chord: ':connections', what: 'register an external API (like QuickBooks) or MCP server once, with its credential, so an operator can later use it — a scheduling API for a patient lookup, a billing API for a report — the credential is captured masked-by-default (eye toggle) and sent straight to the TypeBuild server vault, never stored on this machine and never shown again once saved; the panel lists each connection\'s kind (REST/MCP), endpoint, scope (project/group), and whether a credential is set, with create/edit/delete inline — data still flows client-direct once a connection is wired to a task' },
