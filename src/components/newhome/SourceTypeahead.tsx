// task-e713f307c422 — the typeahead widget for a data-source-backed
// TemplateField (one whose `source` binds a SavedQuery). As the user types we
...
// snapshot up to the modal, which threads the ref onto the created task's
// `data` bag (placeholder keys, per docs/typebuild-data-field-contract.md).
//
// Shared by BOTH the deterministic and copilot New Task paths — the form
// preview panel is the single source of truth, so the same widget lands values
// the same way regardless of which drives the left pane.
//
