...
import { playwrightPromptAddendum } from '../browser/automation';
// task-2e6c926c466c — the app-owned workspace seeder is now SHARED with the
// task-less ad-hoc Ctrl+B browser session (electron/agents/adhoc-browser.ts);
// both call this ONE implementation.
import { ensureTasksWorkspace } from '../agents/tasks-workspace';
import type {
  RunNowOptions,
  SourcedTask,
  TaskSource,
...
// task-3ac8cbe60758 — pure wire-mapping helpers extracted to a testable .mjs
// sibling (mirrors src/components/newhome/taskSchema.mjs's TS-consumer-of-
// .mjs pattern). Byte-identical move; see typebuild-wire.mjs/.d.mts. Only the
// functions this file calls DIRECTLY are imported — mapStatus/isoToMs/
// mapChainFields/mapChainStep are used internally by mapListRow/mapChainRow
// over there and don't need a separate binding here.
import {
  rawStatusOf,
  dateOnly,
  toIso,
  ...
  mapChainRow,
  mapListRow,
  buildCreatePayload as buildCreatePayloadWire,
...
// The app-owned task/browser workspace (ensureTasksWorkspace, TASKS_DIR, the
// permission-grant + seeded browser playbook) moved to
// electron/agents/tasks-workspace.ts (task-2e6c926c466c) so it is shared with
// the task-less ad-hoc Ctrl+B browser session rather than mirrored.
...
// task-3ac8cbe60758 — mapAgentRow/mapResolvedAgent moved to typebuild-wire.mjs
// (pure, unit-tested there). Re-imported above; behavior unchanged.
...
// task-3ac8cbe60758 — ChainDefFieldRow/ChainDefStepRow (the raw per-step wire
// shapes) moved to typebuild-wire.mjs alongside mapChainFields/mapChainStep,
// which are the only consumers; ChainDefRow stays (still typed on two fetch
// call sites below).
type ChainDefRow = {
  id?: string;
  name?: string;
...
// task-3ac8cbe60758 — mapChainFields/mapChainStep/mapChainRow, mapStatus,
// rawStatusOf, dateOnly, toIso, isoToMs, mapResult, mapOutputSchema,
// mapDataKeys, mapMessages, mapPendingQuestion, and mapListRow all moved to
// typebuild-wire.mjs (pure, unit-tested there). Re-imported above; behavior
// unchanged. `OutputSchemaField`/`isOutputSchemaFieldLike` moved with
// mapOutputSchema — other TS call sites in this file still reference the
// field-def shape via `WireOutputSchemaField` (imported below) where needed.
...
type OutputSchemaField = WireOutputSchemaField;
...
export class TypeBuildTaskSource implements TaskSource {
  ...
  // task-3ac8cbe60758 — pure field-mapping moved to typebuild-wire.mjs
  // (buildCreatePayload, unit-tested there); this method is now a thin
  // wrapper so createTask/bulkCreateTasks call sites are unchanged.
  private buildCreatePayload(input: TaskCreate): Record<string, unknown> {
    return buildCreatePayloadWire(input);
  }
  ...
  async updateTemplate(
    ...
    // task-3ac8cbe60758 — pure camelCase→snake_case mapping moved to
    // typebuild-wire.mjs (buildTemplatePatchPayload, unit-tested there).
    const payload = buildTemplatePatchPayload(patch);
    const res = await this.request(
      'PATCH',
      `/chromeext/templates/${encodeURIComponent(id)}`,
      payload,
