...
function playbookBody(): string[] {
  ...
  return [
    ...
    '  `--site` memory is SHARED across machines + teammates (recall it with',
    '  `memory get --site <url>` after you land on a page, BEFORE re-deriving a',
    '  selector); `--task` memory is local to this machine. Capture wins AND',
    '  dead-ends. Delete a shared note with `--site <url> --id <note-id>`.',
    '',
