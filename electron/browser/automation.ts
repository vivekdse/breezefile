...
function playbookBody(): string[] {
  ...
  return [
    ...
    '  API channel → deterministic tool → REPAIR → full agent (FIELD LAYER, then raw).',
    ...
    '   Within it, PERCEIVE-then-act via the FIELD LAYER before reaching for raw',
    '   selectors:',
    '',
    '   FIELD LAYER (JAWS-style form reader — enumerate what is actionable and set',
    '   it by an opaque REF, so you never hand-craft a selector). PREFER it over raw',
    '   snapshot/click/fill on any FORM:',
    `     node ${C} page                       orient: title/url, landmarks, counts`,
    `     node ${C} fields                      one line per field — MINTS the refs`,
    `     node ${C} field <ref>                 a field's contract (kind/options/howToSet)`,
    `     node ${C} field-fill <ref> <value>    set a text/date field (| --data-ref <key>)`,
    `     node ${C} field-select <ref> --pick <label>   commit a choice (native select /`,
    '         combobox / listbox); an async combobox needs --query <text> (| --query-ref <key>).',
    '     REF LIFECYCLE: refs come ONLY from `fields`, and DIE on any page change;',
    '     every field error already names the fix (usually re-run `fields`) — obey it.',
    '     PHI: --data-ref / --query-ref are the field-layer spelling of fill-ref — the',
    '     KEY resolves in-process; the value NEVER enters argv/stdout/your context.',
    '     RECOVERY: when confused, go UP a perception level — re-run `fields` (or',
    '     `page`), don\'t retry the failing action.',
    '',
    '   RAW VERBS (lowest rung — only when the field layer can\'t perceive the control,',
    '   e.g. div-soup with no ARIA roles):',
    `  node ${C} <verb> [args]`,
    ...
    '  whole UI loop. Or use the FIELD LAYER above — `fields` then field-fill/',
    '  field-select — instead of guessing selectors.',
    ...
    '  ONE STORE, ONE LOOKUP: `memory get --site <url>` and the TypeBuild MCP',
    '  `recall_site` read the SAME shared store (as do `memory get --task` and',
    '  `recall_task`). They are NOT two stores — pick ONE per domain/task and do a',
    '  SINGLE lookup; do NOT query both for the same key. The CLI result names the',
    '  MCP tool it already covers in its `covers` field. (If the pre-fetched task',
    '  context bundle was injected at launch, that ALREADY recalled these — do not',
    '  re-run either path for the same key.)',
    '',
    ...
    '',
    '8. CLOSE-OUT — before marking the task done: promote any tool you discovered',
    '  (see PROMOTION HOOK above), record what you learned via submit_task\'s',
    '  `learned` field, `breeze-tools memory add --site` for site-nav learnings,',
    '  submit structured outputs FIRST, then file follow_up tasks for anything',
    '  surfaced. Full ritual + rationale lives in the server global operator doc',
    '  (source of truth) — this is a pointer, not a copy.',
    '',
