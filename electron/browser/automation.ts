...
function playbookBody(): string[] {
  ...
  return [
    ...
    `       API. \`node ${T} memory get --site <url>\` returns any \`api-spec\` notes`,
    '       (NON-PHI, KEYS only: method, path, header/param KEY names, me.* auth',
    '       ref). If one exists, just net-replay/curl it — no rediscovery. When you',
    '       DISCOVER a new API via net-observe/net-replay, the promotion hook emits',
    '       an `http`-channel tool AND records the api-spec note, so the next task',
    '       on that domain skips the browser. An `http`-channel tool (see',
    '       `available`/`help` `channel` field) means its steps ARE the API call.',
    '',
