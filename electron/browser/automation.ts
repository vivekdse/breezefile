...
function playbookBody(): string[] {
  ...
  return [
    ...
    '  On exit 6 (partial) the JSON carries `failed_step` + `resume_from`: re-run',
    `  with \`node ${T} run <id> --resume-from <step>\` to continue WITHOUT`,
    '  re-firing already-done steps (a completed side-effect step is NEVER',
    `  re-run — the runner refuses). \`--dry-run\` prints the step plan offline.`,
    '',
