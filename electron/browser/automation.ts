...
export function playwrightPromptAddendum(): string {
  return [
    '',
    '---',
    'Browser automation (Playwright). Drive the embedded Breeze browser tab by',
    'running this helper via Bash — your actions show live in the tab. This is',
    'already installed: do NOT install Playwright, download browsers, write your',
    'own driver script, or use any claude-in-chrome tools. Use ONLY:',
    '',
    `  node ${BROWSER_CLI} <verb> [args]`,
    '',
    'Verbs:',
    '  open [url]            open a new browser tab (creates one if none)',
    '  goto <url>            navigate the tab',
    '  snapshot [selector]   ARIA tree of the page — your primary "eyes"',
    '  screenshot [path]     PNG of the tab; then Read the file to SEE it',
    ...
    '  url | title | pages   read state / list tabs',
    '',
