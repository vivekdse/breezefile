...
export function browserSubagentMarkdown(): string {
  ...
  return [
    ...
    'When the task lists data placeholders (keys like `patient.ssn`), DO NOT ask',
    'for or type the real values. Fill them with `fill-ref`/`type-ref` using the',
    'KEY — Breeze resolves the value privately. Never read a filled sensitive',
    'field back (eval/snapshot) or screenshot a filled form; that would expose the',
    'value you were given a placeholder to avoid.',
    '',
