@echo off
rem breeze.cmd - Windows runtime shim. Resolves a Node-compatible runtime and
rem runs breeze.mjs. Silent no-op (exit 0) if no runtime is available so this
...
rem   2. System `node` on PATH - for dev (`node bin\breeze.mjs ...`) and for
rem      users who already have Node.
rem
rem The .mjs lives next to this script in both layouts:
rem   ...\bin\breeze.cmd      +  ...\bin\breeze.mjs                 (repo)
rem   ...\resources\breeze.cmd +  ...\resources\breeze.mjs          (installed)

setlocal enabledelayedexpansion

set "HERE=%~dp0"
set "MJS=%HERE%breeze.mjs"
